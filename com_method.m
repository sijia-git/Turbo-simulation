% Comparison of LogMAP & MAXLogMAP
% interleaver size 4000
% 5 interations
% unpunctured
% MAXLogMAP
%
...
% parameters
frame_size=4000; % also interleaver size
interation_number=5;
ispunctured=0; % 0 unpunctured, rate 1/3 ; 1 punctured, rate 1/2
decodermethod=2; % 1 logMAP ; 2 Max-log-MAP
...
errsproMAXLog=zeros(interation_number,length(EbN0_Vec));
...
for k=1:length(EbN0_Vec)
    errsproMAXLog(:,k)=errs(:,k)/(Tx_times(k)*frame_size);
end

savefile='com_method.mat';
save(savefile,'EbN0_Vec','errsproMAXLog')

figure
semilogy(EbN0_Vec,errsproMAXLog(interation_number,:)','-gs')
xlabel('dB')
ylabel('BER')
legend('LogMAP','MAXLogMAP')
title('Comparison of LogMAP & MAXLogMAP')
%**************************************************************************
clock
